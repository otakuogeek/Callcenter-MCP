#!/usr/bin/env python3
"""
Script para importar pacientes del CSV san_gil.csv a la tabla patients (original)
"""
...
def import_patients(csv_file, conn):
    ...
    try:
        with open(csv_file, 'r', encoding='utf-8') as file:
            ...
            print("\n🔄 Importando pacientes del CSV a patients...\n")
            ...
            for row_num, row in enumerate(reader, start=2):
                try:
                    ...
                    cursor.execute("SELECT id FROM patients WHERE document = %s", (document,))
            ...
            print("\n📊 Estadísticas de patients:")
            cursor.execute("SELECT COUNT(*) FROM patients")
            total = cursor.fetchone()[0]
            print(f"   Total de pacientes: {total}")
            
            cursor.execute("SELECT COUNT(*) FROM patients WHERE zone_id = 4")
            zone_4 = cursor.fetchone()[0]
            print(f"   Pacientes zona San Gil (zone_id=4): {zone_4}")
            
            cursor.execute("SELECT COUNT(*) FROM patients WHERE insurance_eps_id = 12")
...
def main():
    ...
    print("🔄 Importando pacientes del CSV a la tabla PATIENTS (original)...")
    print(f"📂 Archivo CSV: {csv_file}")
    print(f"🗄️  Tabla destino: patients")
